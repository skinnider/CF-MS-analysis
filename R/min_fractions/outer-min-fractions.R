...
# parse arguments
parser = ArgumentParser(prog = 'outer-min-fractions.R')
...
chrom_dir = '~/git/CF-MS-analysis/data/chromatograms'
...
# write the raw array
grid_file = "sh/analysis/grids/min_fractions_raw.txt"
...
output_dir = file.path(base_dir, "min_fractions")
...
if (overwrite == F) {
  grid0 = grid %>%
    mutate(output_dir = file.path(base_dir, "min_fractions", accession, 
                                  experiment),
...
write.table(grid0, "sh/analysis/grids/min_fractions.txt",
            quote = F, row.names = F, sep = "\t")

# finally, run the job on whatever system we're on
script = ifelse(system == 'cedar',
                " ~/git/CF-MS-analysis/sh/analysis/min_fractions.sh",
                " ~/git/CF-MS-analysis/sh/analysis/min_fractions.torque.sh")
