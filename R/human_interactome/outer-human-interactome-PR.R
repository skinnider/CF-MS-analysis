...
# parse arguments
parser = ArgumentParser(prog = 'outer-human-interactome-PR.R')
...
# list input files
input_dir = file.path(base_dir, 'human_interactome')
...
write.table(grid0, "sh/analysis/grids/human_interactome_PR.txt",
            quote = F, row.names = F, sep = "\t")

# finally, run the job on whatever system we're on
script = ifelse(system == 'cedar',
                " ~/git/CF-MS-analysis/sh/analysis/human_interactome_PR.sh",
                " ~/git/CF-MS-analysis/sh/analysis/human_interactome_PR.torque.sh")
