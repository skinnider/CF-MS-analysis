...
parser$add_argument('--n_iterations', type = 'integer', default = 10)
...
for (input_file in input_files) {
  ...
  if (ncol(mat) < args$n_fractions) {
    results = data.frame()
    saveRDS(results, output_file)
    stop("not enough fractions")
  }
  
  # repeatedly downsample the matrix 
  for (iteration in seq_len(args$n_iterations)) {
    set.seed(iteration)
    
    # downsample the matrix
    keep_fractions = sample(colnames(mat), args$n_fractions)
    mat0 = mat %>% extract(, keep_fractions)
    ...
    n_fractions = rowSums(!is.na(mat0) & mat0 > 0)
    ...
    if (args$analysis == 'complexes') {
      ...
      if (nrow(pairs0) == 0 | n_distinct(y) < 2) {
        ...
        # append results
        row = data.frame(iteration = iteration,
