...
# parse arguments
parser = ArgumentParser(prog = 'inner-orthogonal-fractionation.R')
...
parser$add_argument('--n_sec', type = 'integer', required = TRUE)
...
expts = read.csv("~/git/CF-MS-searches/data/experiments.csv") %>%
  ...
  filter(Fractionation %in% c('SEC', 'IEX'))
n_sec = args$n_sec
n_iex = args$n_datasets - n_sec
set.seed(args$sample_idx)
datasets = bind_rows(
  expts %>% filter(Fractionation == 'SEC') %>% sample_n(n_sec),
  expts %>% filter(Fractionation == 'IEX') %>% sample_n(n_iex)
)

# read CORUM
corum = read.delim("~/git/network-validation/data/complex/CORUM/complexes_human.txt") %>%
  as_annotation_list('gene_name', 'complex')
complex_proteins = unique(unlist(corum))
...
for (mat_idx in seq_along(mats)) {
  ...
  if (args$feature_select == "best_first") {
    best_features = readRDS("data/analysis/analysis_grid/best_features.rds") %>%
      extract2('combined') %>%
      # keep only one version of each metric
      group_by(metric) %>%
      dplyr::slice(1) %>%
      ungroup() %>%
      arrange(desc(median)) %>%
