...
for (input_file in input_files) {
  ...
  n_fractions = rowSums(!is.na(mat) & mat > 0)
  ...
  # if there aren't enough fractions, write an empty data frame
  if (ncol(mat) < args$min_fractions) {
    results = data.frame()
    saveRDS(results, output_file)
    stop("not enough fractions")
  }
  ...
  # step 5: filter by minimum number of paired observations
  cor[n < args$min_pairs] = NA
  
  # finally, we filter by the minimum number of fractions
  keep = n_fractions >= args$min_fractions
  cor %<>% extract(keep, keep, drop = FALSE)
  meta %<>% extract(keep, )
  
  ## record the number of chromatograms with at least one correlation
  n_obs = sum(colSums(!is.na(cor)) > 1)
  ## record the number of pairs that passed this filter
