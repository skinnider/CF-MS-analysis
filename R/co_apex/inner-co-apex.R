...
# parse arguments
parser = ArgumentParser(prog = 'inner-co-apex.R')
...
parser$add_argument('--metric', type = 'character',
                    choices = c(metrics, 'co_apex'), required = T)
...
for (input_file in input_files) {
  ...
  # read Gaussians
  gaussian_file = file.path(args$gaussian_dir, 
                            paste0("gaussians-", quant_mode, ".rds"))
  gauss = readRDS(gaussian_file)
  
  # keep only protein groups in the matrix with Gaussians fitted
  ## note: for metrics other than co-apex, score the same proteins
  proteins = intersect(names(gauss), rownames(mat))
  gauss %<>% extract(proteins)
  mat %<>% extract(proteins, )
  rownames(meta) = meta$`Majority protein IDs`
  meta %<>% extract(proteins, )
  
  # store number of paired observations in the original matrix
  n = crossprod(!is.na(t(mat)) & t(mat) != 0)
  
  if (args$metric == 'co_apex') {
    cor = -1 * PrInCE::co_apex(gauss)
  } else {
